//#region ../react/build/record-CvRL89xC.js
function e(e, t, n, r) {
...
var t = class {
	...
	constructor(e) {
		...
		super(Object.assign(Object.assign({}, e), {
			...
			renderRecordedAudio: (i = e.renderRecordedAudio) == null || i,
			mediaRecorderTimeslice: e.mediaRecorderTimeslice ?? void 0
		})), this.stream = null, this.mediaRecorder = null, this.dataWindow = null, this.isWaveformPaused = !1, this.lastStartTime = 0, this.lastDuration = 0, this.duration = 0, this.micStream = null, this.recordedBlobUrl = null, this.timer = new r(), this.subscriptions.push(this.timer.on("tick", () => {
	...
	renderMicStream(e) {
		...
		let a = r.frequencyBinCount, o = new Float32Array(a), s = 0;
		this.wavesurfer && (this.originalOptions ??= Object.assign({}, this.wavesurfer.options), this.wavesurfer.options.interact = !1, this.options.scrollingWaveform && (this.wavesurfer.options.cursorWidth = 0, this.wavesurfer.options.normalize = !0, this.wavesurfer.options.maxPeak = 1));
		let c = setInterval(() => {
			if (!this.isWaveformPaused) {
				if (r.getFloatTimeDomainData(o), this.options.scrollingWaveform) {
					let e = Math.floor((this.options.scrollingWaveformWindow || 0) * i), t = 0;
					for (let e = 0; e < a; e++) {
						let n = Math.abs(o[e]);
						n > t && (t = n);
					}
					...
					if (!this.dataWindow) {
						let e = this.options.continuousWaveformDuration ? Math.round(this.options.continuousWaveformDuration * i) : (this.wavesurfer?.getWidth() ?? 0) * window.devicePixelRatio;
						this.dataWindow = new Float32Array(e);
					}
					let e = 0;
					for (let t = 0; t < a; t++) {
		...
		return {
			onDestroy: () => {
				clearInterval(c), n?.disconnect(), t?.close();
			},
			onEnd: () => {
	...
	startMic(t) {
		return e(this, void 0, void 0, function* () {
			...
			let n = this.renderMicStream(e);
			return this.micStream = n, this.unsubscribeDestroy = this.once("destroy", n.onDestroy), this.unsubscribeRecordEnd = this.once("record-end", n.onEnd), this.stream = e, e;
		});
	}
	stopMic() {
		var e, t, n;
	...
	startRecording(t) {
		return e(this, void 0, void 0, function* () {
			let e = this.stream || (yield this.startMic(t));
			this.dataWindow = null;
			let n = this.mediaRecorder || new MediaRecorder(e, {
				mimeType: this.options.mimeType || a.find((e) => MediaRecorder.isTypeSupported(e)),
	...
	destroy() {
		this.applyOriginalOptionsIfNeeded(), super.destroy(), this.stopRecording(), this.stopMic(), this.recordedBlobUrl &&= (URL.revokeObjectURL(this.recordedBlobUrl), null);
	}
...
//# sourceMappingURL=record-CvRL89xC-Djmz1rvI.js.map