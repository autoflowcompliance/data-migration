...
Run with: ``streamlit run app_files/app.py``
Deploy to Streamlit Cloud: Configure main file path as app_files/app.py
"""
...
#
# NOTE: this block uses st.html(), NOT st.markdown(unsafe_allow_html=True).
# st.markdown() runs content through a markdown parser first, and <style>
# blocks do not reliably survive that pass — this is what caused the raw
# CSS to render as literal visible text on the page. st.html() injects
# raw HTML/CSS directly with no markdown parsing. Requires Streamlit >= 1.31.
# --------------------------------------------------------------------------
st.html(
    """
...
if run_clicked and uploaded:
    ...
    try:
        ...
        st.success(
            f"Done — {safe_get(summary, 'rows_in')} rows in, {safe_get(summary, 'rows_out')} out, "
            f"quality score {safe_get(summary, 'quality_score')}%."
        )

        columns = st.columns(5)
        columns[0].metric("Quality score", f"{safe_get(summary, 'quality_score')}%")
        columns[1].metric("Records", safe_get(summary, "rows_out"))
        columns[2].metric("Duplicates removed", safe_get(summary, "duplicates_removed"))
        columns[3].metric("Errors", safe_get(summary, "errors"))
        columns[4].metric("Warnings", safe_get(summary, "warnings"))
        ...
        st.error("The file format looks unusual. Check that it's a valid CSV export and try again.")
        st.caption(f"Technical details: {str(e)}")
