    ...
    if uploaded_file is not None:
        ...
        if process_clicked:
            ...
            try:
                ...
                cols = st.columns(4)
                cols[0].metric("Quality score", f"{summary['quality_score']}%")
                cols[1].metric("Records out", summary["rows_out"])
                cols[2].metric("Duplicates removed", summary["duplicates_removed"])
                ...
                if guarantee_met:
                    ...
                    st.error(
                        f"⚠️ {summary['errors']} unresolved error(s) — this does NOT currently meet "
